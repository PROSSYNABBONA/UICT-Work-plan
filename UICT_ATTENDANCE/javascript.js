document.addEventListener('DOMContentLoaded', () => {
    ...
    const staffData = [
        { id: 1, name: 'John Doe', image: 'adminImage.jpeg', attendance: [1, 1, 1, 0, 1], missed: 1 },
        { id: 2, name: 'Jane Smith', image: 'adminImage.jpeg', attendance: [1, 1, 1, 1, 1], missed: 0 },
        { id: 3, name: 'Alice Johnson', image: 'adminImage.jpeg', attendance: [0, 1, 1, 0, 0], missed: 3 },
        { id: 3, name: 'Alice Johnson', image: 'adminImage.jpeg', attendance: [0, 1, 1, 0, 0], missed: 3 },
        { id: 3, name: 'Alice Johnson', image: 'adminImage.jpeg', attendance: [0, 1, 1, 0, 0], missed: 3 },
        { id: 3, name: 'Alice Johnson', image: 'adminImage.jpeg', attendance: [0, 1, 1, 0, 0], missed: 3 },
        { id: 3, name: 'Alice Johnson', image: 'adminImage.jpeg', attendance: [0, 1, 1, 0, 0], missed: 3 },
    ...
    function updateTable(filter, month) {
        ...
        staffTable.innerHTML = filteredData.map(staff => `
            <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${staff.id}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div class="flex items-center">
                        <img src="${staff.image}" alt="${staff.name}" class="w-8 h-8 rounded-full mr-2">
                        ${staff.name}
                    </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${staff.attendance.join(', ')}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${staff.missed}</td>
            </tr>
        `).join('');
    }
    ...
    saveStaffBtn.addEventListener('click', () => {
        ...
        const newStaff = { id: staffData.length + 1, name: staffName, image: staffImage, attendance: [], missed: 0 };
